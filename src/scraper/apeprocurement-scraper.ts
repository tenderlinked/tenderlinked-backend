    ...
    while (hasNextPage) {
        ...
        const rowsData = await page.evaluate(() => {
            ...
            return rows.map((tr, index) => {
                ...
                const actionTd = tds[8];
                let downloadScript = '';
                let detailsScript = '';
                if (actionTd) {
                    const links = Array.from(actionTd.querySelectorAll('a'));
                    const downloadBtn = links.find(a => a.getAttribute('onclick')?.includes('fnviewTenderDocuments'));
                    if (downloadBtn) {
                        downloadScript = downloadBtn.getAttribute('onclick') || '';
                    }
                    
                    const dBtn = links.find(a => a.getAttribute('onclick')?.includes('viewBtn'));
                    if (dBtn) {
                        detailsScript = dBtn.getAttribute('onclick') || '';
                    }
                }
        ...
        for (const row of rowsData) {
            ...
            const tenderObj = {
                ...
                level: "STATE",
                organisation: toTitleCase(row.department),
            ...
            try {
                ...
                if (row.detailsScript) {
                    ...
                    const detailsPagePromise = new Promise<any>(x => browser.once('targetcreated', async target => {
                        const newPage = await target.page();
                        if (newPage) x(newPage);
                    }));
                    
                    const timeoutPromise = new Promise<any>(x => setTimeout(() => x(null), 15000));

                    await page.evaluate((script) => {
                        try { eval(script); } catch(e) {}
                    }, row.detailsScript);

                    const detailsPage = await Promise.race([detailsPagePromise, timeoutPromise]);
                    if (detailsPage) {
                        try {
